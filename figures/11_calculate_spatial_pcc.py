...
for param in params:
    try:
        print(param["model"])
        base_path = os.path.join(os.environ.get('DATA_MODELS_PATH', '/groups/assafza_group/assafza'), "full_cells_fovs/train_test_list/spatial_pcc/{}".format(param["model"].split('/')[-1]))
        create_dir_if_not_exist(base_path)
        # ds_path = "{}/merged_dataset.csv".format(base_path)
        ds_path = os.path.join(os.environ.get('DATA_MODELS_PATH', '/groups/assafza_group/assafza'), 'full_cells_fovs/train_test_list/Endosomes/image_list_with_metadata__with_efficacy_scores_full.csv')
