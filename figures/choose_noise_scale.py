...
def plot_noise_scale_analysis():
    # Enhanced setup for 9 subplots, each with additional annotations for the first x-value where PCC < 0.15
    ...
    for i, ax in enumerate(axs.flatten()):
        try:
            ...
            ax.axhline(y=0.15, color='r', linestyle='--', label='Threshold: PCC=0.15')
            ax.set_title(params[i]["organelle"])
            ax.set_xlabel('Noise Standard Deviation')
            ax.set_ylabel('PCC')
            
            # Find the first x-value where PCC < 0.15 and annotate
            below_threshold = noise_std[pcc < 0.15]
            if below_threshold.size > 0:  # Check if there's any value below the threshold
                ...
                # ax.annotate(f'{first_below}', xy=(first_below, 0.15), xytext=(first_below, 0.155),
