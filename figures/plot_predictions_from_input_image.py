...
def generate_prediction_image(dataset, params, save_dir):
    ...
    fig.suptitle("Predictions from pipeline 4.4", fontsize=figure_config["title"], fontname=figure_config["font"],y=0.95)
    ...
    for i, param in enumerate(params):
        ...
        axes[row, col].set_title(param['organelle'],fontsize=figure_config["organelle"], fontname=figure_config["font"])
        # axes[row, col].text(0.05, 0.95, title_text, ha='left', va='top', transform=axes[row, col].transAxes,
        #                     color='white', fontsize=10)
    
    final_fig_path = f"{save_dir}/input_and_predictions_all.png"
    plt.tight_layout()
    plt.savefig(final_fig_path)
    plt.show()

save_dir = "/sise/home/lionb/figures/predictions/Endosomes_4" #Endosomes_4
