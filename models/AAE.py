...
def get_decoder(latent_dim,output_size,layer_dim,filters,name="decoder"):
        ...
        output = conv_layer(filters=1, kernel_size=3,strides=1,padding='same',kernel_initializer='glorot_normal',activation='sigmoid',name="{}_conv_{}_out".format(name,layer_dim[0]))(x)
...
class AAE(keras.Model):
    def __init__(self, encoder, decoder, generator_input_size, discriminator_latent, discriminator_image, **kwargs):
        ...
        self.generator = keras.Model(generator_input,self.decoder(self.encoder(generator_input)[-1:]),name="generator")
