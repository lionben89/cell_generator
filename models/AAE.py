...
def get_encoder(input_size,latent_dim,name="encoder"):
        ...
        x=input
        x = conv_layer(filters=filters,kernel_size=3,strides=1,padding='same',activation="relu",kernel_initializer='glorot_normal',name="{}_conv_{}".format(name,layer_dim[0]))(x)
        if (gv.batch_norm):
        ...
        while layer_dim[0] > 1:
            ...
            filters = filters * 2
            x = conv_layer(filters=filters,kernel_size=3,strides=2,padding='same',activation="relu",kernel_initializer='glorot_normal',name="{}_conv1_{}".format(name,layer_dim[0]))(x)
            if (gv.batch_norm):
                x = keras.layers.BatchNormalization(name="{}_bn1_{}".format(name,layer_dim[0]))(x)
                
            x = conv_layer(filters=filters,kernel_size=3,strides=1,padding='same',activation="relu",kernel_initializer='glorot_normal',name="{}_conv2_{}".format(name,layer_dim[0]))(x)
            if (gv.batch_norm):
        ...
        z = keras.layers.Dense(latent_dim, kernel_regularizer="l2", name="{}_z".format(name))(x)
...
def get_decoder(latent_dim,output_size,layer_dim,filters,name="decoder"):
        ...
        x = conv_layer(filters=filters,kernel_size=3,strides=1,padding='same',activation="relu",kernel_initializer='glorot_normal',name="{}_convt_{}".format(name,layer_dim[0]))(x)
        if (gv.batch_norm):
            x = keras.layers.BatchNormalization(name="{}_bn_{}".format(name,layer_dim[0]))(x)
        ...
        while layer_dim[0] < output_size[0]:
            ...
            filters = filters // 2
            x = conv_layer(filters=filters,kernel_size=3,strides=2,padding='same',activation="relu",kernel_initializer='glorot_normal',name="{}_convt1_{}".format(name,layer_dim[0]))(x)
            if (gv.batch_norm):
                x = keras.layers.BatchNormalization(name="{}_bn1_{}".format(name,layer_dim[0]))(x)
                
            x = conv_layer(filters=filters,kernel_size=3,strides=1,padding='same',activation="relu",kernel_initializer='glorot_normal',name="{}_convt2_{}".format(name,layer_dim[0]))(x)
            if (gv.batch_norm):
                x = keras.layers.BatchNormalization(name="{}_bn2_{}".format(name,layer_dim[0]))(x)
      

        ## last conv same resulotion
        output = conv_layer(filters=1, kernel_size=3,strides=1,padding='same',activation='relu',kernel_initializer='glorot_normal',name="{}_conv_{}_out".format(name,layer_dim[0]))(x)
...
def get_discriminator_image(input_size,name="discriminator_image"):
        ...
        ## first conv same resulotion
        x = input
        x = conv_layer(filters=filters,kernel_size=3,strides=1,padding='same',activation="relu",kernel_initializer='glorot_normal',name="{}_conv_{}".format(name,layer_dim[0]))(input)
        if (gv.batch_norm):
            x = keras.layers.BatchNormalization(name="{}_bn_{}".format(name,layer_dim[0]))(x)
        ...
        while layer_dim[0] > 1:
            layer_dim = np.int32(layer_dim / 2)
            filters = filters * 2
            x = conv_layer(filters=filters,kernel_size=3,strides=2,padding='same',activation="relu",kernel_initializer='glorot_normal',name="{}_conv_{}".format(name,layer_dim[0]))(x)
            if (gv.batch_norm):
                x = keras.layers.BatchNormalization(name="{}_bn_{}".format(name,layer_dim[0]))(x)
...
class AAE(keras.Model):
    ...
    def train_step(self, data, train=True):
        ...
        with tf.GradientTape() as tape:
            ...
            reconstruction_loss = tf.reduce_mean(
                tf.reduce_sum(
                    0.1*tf.square(data[1]-reconstruction),axis=(1,2,3)
                ),axis=0
            )
            
            # reconstruction_loss = tf.reduce_mean(keras.losses.mean_squared_error(data[1], reconstruction),axis=(1,2,3))
            ...
            total_loss = 1*reconstruction_loss + 10*generator_loss
        ...
        with tf.GradientTape() as tape:
            ...
            if (train):
                grads = tape.gradient(10*encoder_loss, self.encoder.trainable_weights)
