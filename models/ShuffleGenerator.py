...
class ShuffleGenerator(keras.Model):
    ...
    def compile(self, d_optimizer, g_optimizer, unet_loss_weight=0.01, discriminator_loss_weight=1, reconstruction_loss_weight=0.05):
    ...
    def train_step(self, data, train=True):
        ...
        unet_shuffled = self.unet(input_shuffled)
        
        # Train generator to fool discriminator_image and create target
        with tf.GradientTape() as tape:
            adapted_image = self.generator([data_0,input_shuffled])
            ...
            unet_loss = tf.reduce_mean(
                tf.reduce_sum(
                    keras.losses.mean_squared_error(unet_shuffled, unet_predictions),axis=(1,2,3)
                ),axis=0
            )
            ...
            total_loss =  generator_loss*self.discriminator_loss_weight + reconstruction_loss*self.reconstruction_loss_weight + unet_loss*self.unet_loss_weight
