    ...
    for res_log2 in range(start_res_log2, target_res_log2 + 1):
        ...
        for phase in ["TRANSITION", "STABLE"]:
            if res == start_res and phase == "TRANSITION":
                continue
            patch_size = (16,res,res,1)
            train_dl = DataGen(gv.train_ds_path,gv.input,gv.target,batch_size = gv.batch_size, num_batches = 64, patch_size=patch_size,min_precentage=0,max_precentage=0.95,resize=True,delete_cahce=True)
            ...
            print(phase)
            style_gan.fit(
                train_dl,steps_per_epoch = 200, epochs=1, callbacks=[ckpt_cb]
            )

            if display_images:
                images = style_gan({"z": val_z, "noise": val_noise, "alpha": 1.0})
                for i in range(images.shape[0]):
                    ImageUtils.imsave(images[i],"{}/predictions/input_patch_{}.tiff".format(gv.stylegan_path,i))
                    
stylegan = StyleGAN(target_res=256)
train(stylegan)
